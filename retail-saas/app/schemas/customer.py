...
class CustomerCreate(CustomerBase):
    ...
    @field_validator("address")
    @classmethod
    def validate_create_address(cls, v: Optional[str]) -> Optional[str]:
        if v is None:
            return None
    ...
    @field_validator("birthday")
    @classmethod
    def validate_create_birthday(cls, value: date) -> date:
        if value > date.today():
            raise ValueError("Birthday cannot be in the future")
...
class WalletCreditRequest(BaseModel):
    ...
    @classmethod
    def validate_reference(cls, v: Optional[str]):
        if v is None:
            return None
    ...
    @classmethod
    def validate_remarks(cls, v: Optional[str]):
        if v is None:
            return None
...
class CommunicationCreate(BaseModel):
    customer_id: Optional[int] = Field(default=None, gt=0)
    channel: Optional[Literal["whatsapp", "sms", "email"]] = None
    communication_type: Optional[Literal["SMS", "WHATSAPP", "EMAIL"]] = None
    message: str = Field(
        min_length=1,
        max_length=5000,
    )
    ...
    def sync_communication_type(self):
        if self.communication_type is None and self.channel is not None:
            self.communication_type = self.channel.upper()

        if self.channel is None and self.communication_type is not None:
            self.channel = self.communication_type.lower()
...
class ReferralCreate(BaseModel):
    ...
    def validate_referral(self):
        if (
            ...
            and self.customer_id == self.referred_customer_id
        ):
    ...
    def validate_note(cls, v: str):
        ...
        for pattern in dangerous_patterns:
            if re.search(pattern, v, re.IGNORECASE):
                raise ValueError(
                    "Note contains invalid or dangerous HTML/script content"
                )
...
class CampaignSendRequest(BaseModel):
    ...
    channel: Optional[Literal["whatsapp", "sms", "email"]] = None
    customer_ids: Optional[list[int]] = Field(default=None, min_length=1)
    communication_type: Optional[str] = None
    ...
    @classmethod
    def validate_customer_ids(cls, v: Optional[list[int]]):
        if v is None:
            return None
    ...
    def validate_comm_type(cls, v):
        ...
        if v not in ("SMS", "WHATSAPP", "EMAIL"):
            raise ValueError(
                "Invalid communication type. Allowed values: SMS, WHATSAPP, EMAIL"
            )
